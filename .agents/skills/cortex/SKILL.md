...
Por padrão, todos os diagramas gerados pelo Archify no PCL AEOS (títulos, descrições de nós, rótulos de conexões, mensagens de sequência, fases e legendas) devem ser redigidos em **Português (Brasil)**:
- Utilize terminologia técnica clara e natural (ex: `Entrada de Dados`, `Validação de Token`, `Serviço de Autenticação`, `Banco de Dados Relacional`, `Fila de Eventos`, `Sucesso`, `Falha`).
- Manter siglas e termos técnicos universais quando aplicável (ex: `API Gateway`, `PostgreSQL`, `Docker`, `OAuth 2.0`, `JWT`, `JSON-RPC`).
